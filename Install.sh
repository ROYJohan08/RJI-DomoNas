...
echo "Creation des fichiers de travail : PASS" > /etc/RJIDomoNas/Log/Install.log
...
apt-get autoclean -y
apt-get clean -y
echo "Installation des mises à jours et nettoyage des paquets : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation de docker : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation de Git, net-tools, iperf et des driver gpgpu : PASS" >> /etc/RJIDomoNas/Log/Install.log

apt-get install smartmontools -y
echo "Installation des utilitaires de disks : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
service smbd start
echo "Installation et configuration de samba : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation et configuration de glances : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
if test -f "$FILE"; then
    ...
    echo "Récupération du fichier de mot de passe par defaut : PASS" >> /etc/RJIDomoNas/Log/Install.log
fi
...
if test -f "$FILE"; then
    ...
    echo "Remplacement du fichier de mot de passe par une sauvegarde : PASS" >> /etc/RJIDomoNas/Log/Install.log
else 
    
fi

sudo -u "$SUDO_USER" bash /etc/RJIDomoNas/InstallDrives.sh
echo "Installation de tous les disques : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Récupération et lancement de Docker.sh : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
source /root/.bashrc
echo "Modification des alias : PASS" >> /etc/RJIDomoNas/Log/Install.log

wget -O /etc/RJIDomoNas/Update.sh https://github.com/ROYJohan08/DomotikHomeNas/raw/main/Docs/Update.sh
echo "Récupération du systeme de mises à jours : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Récupération et installation des taches cron : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Supression de la veille : PASS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Instation de python : PASS" >> /etc/RJIDomoNas/Log/Install.log

wget -O /etc/RJIDomoNas/Archive.sh https://raw.githubusercontent.com/ROYJohan08/RJI-DomoNas/main/Docs/Archive.sh
echo "Installation du systeme d'archivage : PASS" >> /etc/RJIDomoNas/Log/Install.log
