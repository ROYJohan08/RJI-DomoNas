...
echo "Creation des fichiers de travail: SUCCESS" > /etc/RJIDomoNas/Log/Install.log
...
apt-get autoclean -y
apt-get clean -y
echo "Installation des mises à jours et nettoyage des paquets: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation de docker: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation de Git, net-tools, iperf et des driver gpgpu: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log

apt-get install smartmontools -y
echo "Installation des utilitaires de disks: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
service smbd start
echo "Installation et configuration de samba: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Installation et configuration de glances: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Instalation des disques par defaut : WARNING" >> /etc/RJIDomoNas/Log/Install.log

FILE=/etc/RJIDomoNas/credentials.sh
if test -f "$FILE"; then
    echo "Le fichier de mot de passe est déjà présent : WARNING" >> /etc/RJIDomoNas/Log/Install.log
else 
    wget -O /etc/RJIDomoNas/credentials.sh https://github.com/ROYJohan08/DomotikHomeNas/raw/main/Docs/credentials.sh
    echo "Récupération du fichier de mot de passe par defaut: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
fi
...
if test -f "$FILE"; then
    ...
    echo "Remplacement du fichier de mot de passe par une sauvegarde: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
else 
    
fi

sudo -u "$SUDO_USER" bash /etc/RJIDomoNas/InstallDrives.sh
echo "Installation de tous les disques: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Récupération et lancement de Docker.sh: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
source /root/.bashrc
echo "Modification des alias: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log

wget -O /etc/RJIDomoNas/Update.sh https://github.com/ROYJohan08/DomotikHomeNas/raw/main/Docs/Update.sh
echo "Récupération du systeme de mises à jours: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Récupération et installation des taches cron: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Supression de la veille: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
...
echo "Instation de python: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log

wget -O /etc/RJIDomoNas/Archive.sh https://raw.githubusercontent.com/ROYJohan08/RJI-DomoNas/main/Docs/Archive.sh
echo "Installation du systeme d'archivage: SUCCESS" >> /etc/RJIDomoNas/Log/Install.log
echo "Fin du SCRIPT" >> /etc/RJIDomoNas/Log/Install.log
