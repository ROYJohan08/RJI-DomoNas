...
case "$1" in
        "lamp")
                ...
                sudo docker pull php:8.2-apache || echo "⚠️ Avertissement : Impossible de télécharger la dernière image, utilisation de l'image locale."
                sudo docker run -d \
                        --name lamp \
        ...
        "filebrowser")
                ...
                sudo docker pull gtstef/filebrowser:latest
                sudo docker run -d \
                        --name filebrowser \
                        --restart=unless-stopped \
                        -e TZ=CET \
        ...
        "nextcloud")
                ...
                remove_container nextcloud
                sudo docker pull nextcloud:latest
                docker run -d \
                          --name nextcloud \
                          -p 1205:80 \
                          -v /media/Runable/NC-Data:/var/www/html \
                          -v /media/Docs01/NextCloud:/mnt/stockage_perso \
                          -v /media/Docs01/Photographies:/mnt/photos \
                          --restart unless-stopped \
                          nextcloud:latest
        ;;
        "vaultwarden")
                remove_container vaultwarden
