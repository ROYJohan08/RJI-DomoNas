...
case $1 in
	"all")
		...
		sudo docker run -d --name Seedbox --privileged --restart=unless-stopped -e PUID=$USER_ID -e PGID=$GROUP_ID -e TZ=CET -e USER=$User -e PASS=$Pass -e TRANSMISSION_DOWNLOAD_DIR=/data/SeedBox -e TRANSMISSION_WEB_HOME=/config/GUI -p $SBPort:9091 -p 51413:51413 -p 51413:51413/udp -v $SBConfig:/config -v $SBData:/downloads lscr.io/linuxserver/transmission:latest
	...
	;;
	"readeck")
		sudo docker run -d --name Readeck --privileged --restart=unless-stopped -e TZ=CET -p $RSPort:80 -v $RSData:/www/FreshRSS/data -e 'CRON_MIN=1,31' freshrss/freshrss
    ;;
esac
