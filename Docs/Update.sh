...
echo "Creation des fichiers de travail : PASS" > /etc/RJIDomoNas/Log/Update.sh
...
apt-get autoclean -y
apt-get clean -y
echo "Installation des mises à jours et nettoyage des paquets : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
echo "Installation de docker : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
echo "Installation de Git, net-tools, iperf et des driver gpgpu : PASS" >> /etc/RJIDomoNas/Log/Update.sh

apt-get install smartmontools -y
echo "Installation des utilitaires de disks : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
service smbd start
echo "Installation et configuration de samba : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
echo "Installation et configuration de glances : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
echo "Instalation des disques par defaut : WARNING" >> /etc/RJIDomoNas/Log/Update.sh

FILE=/etc/RJIDomoNas/credentials.sh
if test -f "$FILE"; then
    echo "Le fichier de mot de passe est déjà présent : WARNING" >> /etc/RJIDomoNas/Log/Update.sh
else 
    wget -O /etc/RJIDomoNas/credentials.sh https://github.com/ROYJohan08/DomotikHomeNas/raw/main/Docs/credentials.sh
    echo "Récupération du fichier de mot de passe par defaut : PASS" >> /etc/RJIDomoNas/Log/Update.sh
fi
...
if test -f "$FILE"; then
    ...
    echo "Remplacement du fichier de mot de passe par une sauvegarde : PASS" >> /etc/RJIDomoNas/Log/Update.sh
else 
    
fi

sudo -u "$SUDO_USER" bash /etc/RJIDomoNas/InstallDrives.sh
echo "Installation de tous les disques : PASS" >> /etc/RJIDomoNas/Log/Update.sh

wget -O /etc/RJIDomoNas/Docker.sh https://github.com/ROYJohan08/DomotikHomeNas/raw/main/Docs/Docker.sh
echo "Récupération de Docker.sh : PASS" >> /etc/RJIDomoNas/Log/Update.sh
...
source /root/.bashrc
echo "Modification des alias : PASS" >> /etc/RJIDomoNas/Log/Update.sh

wget -O /etc/RJIDomoNas/Archive.sh https://raw.githubusercontent.com/ROYJohan08/RJI-DomoNas/main/Docs/Archive.sh
echo "Installation du systeme d'archivage : PASS" >> /etc/RJIDomoNas/Log/Update.sh
